...
final class PersonnelTable extends PowerGridComponent
{
    ...
    public function datasource(): Builder
    {
        return Personnel::query()
            ->select([
                ...
                'address',
                'is_admin',
                DB::raw("CONCAT(first_name, ' ', mid_initial, ' ', last_name) AS full_name")
            ])
            ->orderBy('id', 'ASC');
    }
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ->add('id')
            ->add('full_name')
            ->add('position', function ($personnel) {
                return $personnel->is_admin ? 'Manager' : 'Sales';
            })
            ->add('created_at');
    }
    ...
    public function actions(\App\Models\Personnel $row): array
    {
       return [
                Button::add('edit')
                ...
                ->openModal('modal.personnel-form', ["id" => $row->id, "action" => 'personnels.update']),
                ...
                ->openModal('modal.personnel-form', ["id" => $row->id, "action" => 'personnels.destroy']),
        ];
    }

    public function header(): array
    {
        $action = URL::route('personnels.store');
        return [
            Button::add('add')
                ->slot('Add Personnels')
                ->class('inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150')
                ->openModal('modal.personnel-form', ["id" => null, "action" => $action]),
        ];
    }
}
